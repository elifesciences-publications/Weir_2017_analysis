...
input_data <- read_csv('/path/to/Weir_2017_analysis/figure_6/Fig_S6J_pex12_data.csv')
...
input_data$bsub_yfp <- input_data$yfp_mean - exp(summ$mean_log_yfp[summ$substrate == 'noyft'])
input_data$log_bsub_yfp <- log(input_data$bsub_yfp)

# summarize data for plotting
summ_for_plot <- ddply(.data=input_data, .variables=c('allele','auxin','substrate','time_in_hr'),
                       summarize,
...
# normalize to t=0 +auxin
for (s in unique(summ_for_plot$substrate)){
  for (a in unique(summ_for_plot$allele)){
    summ_for_plot$norm_log_yfp[summ_for_plot$substrate == s & summ_for_plot$allele == a] <- summ_for_plot$mean_log_yfp[summ_for_plot$substrate == s & summ_for_plot$allele == a] - summ_for_plot$mean_log_yfp[summ_for_plot$substrate == s & summ_for_plot$allele == a & summ_for_plot$auxin == 'plusauxin' & summ_for_plot$time_in_hr == 0]  
  }
}
# plot.
ggplot(subset(summ_for_plot, substrate != 'noyft' & auxin == 'plusauxin'),
       aes(x = factor(time_in_hr), y = norm_log_yfp, color = allele, group=interaction(allele,auxin))) +
  facet_grid(substrate ~ .) +
  geom_line() + geom_point() +
  geom_errorbar(aes(ymin = norm_log_yfp-se_log_yfp, ymax = norm_log_yfp+se_log_yfp), width=0.5) +
...
#ggsave('/path/to/fig/S6J_pex12/plot.pdf', device = cairo_pdf(width = 3, height = 3), useDingbats = F)
